...
create or replace trigger tg_login before
   insert on login
   referencing
      new as new
   for each row
begin
     -- Gera o próximo código de login usando a sequence
   ...
   :new.senha := fn_cripto(
    ...
    -- Registra o acesso na tabela ACESSO
   insert into acesso values ( systimestamp,
                               :new.cod_login );
end;
/

create or replace trigger tg_valida_parametros before
   insert on acesso
   referencing
      new as new
   for each row
declare
   valcodlogin number;
begin
        -- Verifica se o login existe na tabela LOGIN
   select cod_login
     into valcodlogin
     from login
    where cod_login = :new.cod_login;

        -- Se o login não existir, gera um erro
   if valcodlogin is null then
...
create or replace function fn_cripto (
   cod_login number,
   senha     varchar2
) return varchar2 as
    -- Define o tipo da matriz e o tamanho de cada posição
   type array_linhas is
      table of varchar2(1) index by pls_integer;
   ...
   resultado3  varchar2(4000) := ''; -- Armazenará o resultado final com incremento
   incremento  number := 0; -- Valor a ser acrescido (centésimo de segundo)
   valor_ascii number := 0; -- Valor ASCII temporário
   novo_valor  number := 0; -- Valor após incremento
begin 
    -- * 1ª Etapa **
    -- Loop para percorrer os caracteres da senha
   for i in 1..length(senha) loop
        -- Switch case para preencher a matriz
      case linha_atual
         when 1 then
            linha1(coluna1) := substr(
               senha,
               i,
               1
            );
   ...
   for z in linha3.first..linha3.last loop
    ...
    -- 2ª Etapa: Conversão para ASCII (3 dígitos concatenados) --
   for i in 1..length(resultado1) loop
    ...
    -- 3ª Etapa: Incremento com centésimo de segundo --
   ...
   return resultado3; -- Retorna a senha criptografada final
end;
/

create or replace trigger tg_cripto before
   insert on login
   referencing
      new as new
   for each row
...
create or replace procedure pr_acesso (
   par_login varchar2,
   par_senha varchar2
) as
   cod_login       number;
...
begin
   ...
   select max(to_number(to_char(
    ...
    -- Se não há registros de acesso, usa 0
   ...
   for i in 0..( trunc((length(senha_cripto) / 3)) - 1 ) loop
    ...
    -- Converte blocos de 3 dígitos ASCII de volta para caracteres
   for i in 0..( trunc((length(resultado3) / 3)) - 1 ) loop
    ...
    -- Reorganiza os caracteres de volta à ordem original
   ...
   senha_decripto := resultado1;
    ...
    -- Verifica se a senha decriptografada confere com a senha fornecida
   if senha_decripto = par_senha then
        -- Registra o novo acesso
      insert into acesso values ( systimestamp,
                                  cod_login );

        -- Gera nova senha criptografada baseada no SYSTIMESTAMP atual
      update login
         set
         senha = fn_cripto(
            cod_login,
      ...
      dbms_output.put_line('Acesso autorizado para o usuário: '
                           || par_login
...
exception
   when others then
      ...
      raise_application_error(
         -20002,
         'Erro durante a validação: ' || sqlerrm
      );
end;
/
